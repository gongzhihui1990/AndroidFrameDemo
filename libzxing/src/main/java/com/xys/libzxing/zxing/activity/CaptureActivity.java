...
public final class CaptureActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private boolean onCameraResume = false;
    private BroadcastReceiver scannerReceiver = new BroadcastReceiver() {
        @RequiresApi(api = Build.VERSION_CODES.KITKAT)
        @Override
        public void onReceive(Context context, Intent intent) {
            if (Objects.equals(intent.getAction(), "com.android.server.scannerservice.broadcast")) {
                String key = "scannerdata";
                if (intent.hasExtra(key)) {
                    String barcode = intent.getStringExtra(key).trim();
                    inactivityTimer.onActivity();
                    beepManager.playBeepSoundAndVibrate();

                    Intent resultIntent = new Intent();
                    Bundle bundle = new Bundle();
                    bundle.putString(String.class.getName(), barcode);
...
//        scanPreview = findViewById(R.id.capture_preview);
        findViewById(R.id.barTitleBack).setOnClickListener(new View.OnClickListener() {
            @Override
...
//        onCameraResume();
    ...
    private void onCameraResume() {
        Log.d(TAG, "onCameraResume start " + isHasSurface);
        ...
        if (isHasSurface) {
            ...
            // camera.
            Log.d(TAG, "onCameraResume addCallback");
            scanPreview = new SurfaceView(getBaseContext());
            ViewGroup.LayoutParams lp = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            scanContainer.addView(scanPreview, 0, lp);
            scanPreview.getHolder().addCallback(this);
        }

        inactivityTimer.onResume();
        Log.d(TAG, "onCameraResume end");
    ...
    protected void onPause() {
        ...
        if (!isHasSurface) {
            Log.d(TAG, "onPause removeCallback");
    ...
    public void handleDecode(Result rawResult, Bundle bundle) {
        ...
        bundle.putString(String.class.getName(), rawResult.getText());
        resultIntent.putExtras(bundle);
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        if (surfaceHolder == null) {
            Log.d(TAG, "No SurfaceHolder provided");
