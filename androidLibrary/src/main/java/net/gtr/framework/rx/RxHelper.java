...
public final class RxHelper {
     ...
     */
    public static <T> void bindOnNull(@NonNull Observable<T> observable) {
     ...
     * @return
     */
    public static Observable<Long> countdown(int time, long period, TimeUnit unit) {
        if (time < 0) {
            time = 0;
        }
        final int countTime = time;
        return Observable.interval(0, period, unit).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).map(new Function<Long, Long>() {
            //通过map()将0、1、2、3...的计数变为...3、2、1、0倒计时
            @Override
            public Long apply(@NonNull Long increaseTime) throws Exception {
     ...
     * @return
     */
    public static void delayRun(int time, TimeUnit unit, Runnable delay, ApplicationObserverResourceHolder holder) {
        bindOnUI(delay(time, unit, delay), new ProgressObserverImplementation<Runnable>(holder) {
            @Override
     ...
     * @return
     */
    public static void delayRun(int time, TimeUnit unit, Runnable delay) {
        bindOnUI(delay(time, unit, delay), new Observer<Runnable>() {
            @Override
